 ...
 * Examples:
 * ```
 * var result = await api.addMPNews(agentid, articles);
 * ```
 * Result:
 * ```
 ...
 * Examples:
 * ```
 * var result = await api.updateMPNews(agentid, media_id, articles);
 * ```
 * Result:
 * ```
 * {"errcode": "0","errmsg": "ok","media_id": "2-G6nrLmr5EC3MMfasdfb_-zK1dDdzmd0p7"}
 ...
 * Examples:
 * ```
 * var result = await api.addMaterial(agentid, 'filepath', type);
 * ```
 * Result:
 * ```
 ...
 * Examples:
 * ```
 * var ret = await api.getMaterial(agentid, mediaId);
 * ```
 ...
 * Examples:
 * ```
 * var ret = await api.delMaterial(agentid, mediaId);
 * ```
 ...
 * Examples:
 * ```
 * var ret = await api.countMaterial(agentid);
 * ```
 * {
 *   "errcode": 0, 
 *   "errmsg": "ok",
 ...
 * Examples:
 * ```
 * var ret = await api.batchgetMaterial(agentid, type, offset, count);
 * ```
 * - `result`,  {"errcode": 0, "errmsg": "deleted"}
 *
 * @param {Integer} agentid 企业应用的id，整型。可在应用的设置页面查看
