 ...
 * Examples:
 * ```
 * var result = await api.batchInviteUser(to, callback);
 * ```
 * To:
 * ```
 * {
 ...
 * Examples:
 * ```
 * var result = await api.batchSyncUser(mediaId, callback);
 * ```
 *
 * ```
 * TaskCb:
 ...
 * Examples:
 * ```
 * var result = await api.batchReplaceUser(mediaId, callback);
 * ```
 *
 * ```
 * TaskCb:
 ...
 * Examples:
 * ```
 * var result = await api.batchReplaceParty(mediaId, callback);
 * ```
 *
 * ```
 * TaskCb:
 ...
 * Examples:
 * ```
 * var result = await api.batchGetResult(jobid);
 * ```
 *
 * 返回结果参考微信的官方文档
